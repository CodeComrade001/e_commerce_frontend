...
import { HomeAvailableProductProps, Product } from "../pages/Home";

export default function HomeAvailableProduct({
  group,
}: HomeAvailableProductProps): JSX.Element {
  const [loadProducts, setLoadProducts] = useState<Product[]>([])

  useEffect(() => {
    setLoadProducts(group.products)
  }, [group])
